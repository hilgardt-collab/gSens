...
class LCARSComboDisplayer(ComboBase):
    ...
    def _draw_frame(self, ctx, width, height, position):
        ...
        if position == "top":
            ctx.new_path()
            ctx.move_to(width, h_bar_h + shoulder_r) 
            ctx.line_to(width, h_bar_h) 
            ctx.line_to(shoulder_r, 0) # Top edge
            ctx.arc_negative(shoulder_r, shoulder_r, shoulder_r, -math.pi/2, math.pi) # outer shoulder
            ...
            ctx.arc(v_bar_w - shoulder_r, h_bar_h + shoulder_r, shoulder_r, 0, -math.pi/2) # inner shoulder
            ctx.line_to(width, h_bar_h)
            ctx.close_path()
            ctx.fill()
        else: # bottom
            ctx.new_path()
            ctx.move_to(width, height - h_bar_h - shoulder_r)
            ctx.line_to(width, height - h_bar_h)
            ctx.line_to(shoulder_r, height)
            ctx.arc(shoulder_r, height - shoulder_r, shoulder_r, math.pi/2, math.pi)
            ctx.line_to(0, 0)
            ctx.line_to(v_bar_w, 0)
            ctx.line_to(v_bar_w, height - h_bar_h - shoulder_r)
            ctx.arc_negative(v_bar_w - shoulder_r, height - h_bar_h - shoulder_r, shoulder_r, 0, -math.pi/2)
            ctx.line_to(width, height - h_bar_h)
            ctx.close_path()
            ctx.fill()
    ...
    def _draw_segments(self, ctx, height, position):
        ...
        for i in range(1, num_segments + 1):
            ...
            # Create a clipping path for the segment that respects the shoulder curve
            ctx.new_path()
            if position == "top":
                ctx.move_to(0, current_y)
                ctx.line_to(v_bar_w, current_y)
                if i == 1: # Clip first segment against inner shoulder
                    ctx.line_to(v_bar_w, h_bar_h + shoulder_r)
                    ctx.arc(v_bar_w - shoulder_r, h_bar_h + shoulder_r, shoulder_r, 0, -math.pi/2)
                    ctx.line_to(0, h_bar_h)
                else:
                     ctx.line_to(v_bar_w, current_y + segment_height)
                     ctx.line_to(0, current_y + segment_height)
            else: # bottom
                start_y = current_y
                end_y = current_y + segment_height
                ctx.move_to(0, start_y)
                ctx.line_to(v_bar_w, start_y)
                if i == num_segments: # Clip last segment
                    ctx.line_to(v_bar_w, height - h_bar_h - shoulder_r)
                    ctx.arc_negative(v_bar_w - shoulder_r, height - h_bar_h - shoulder_r, shoulder_r, 0, -math.pi/2)
                    ctx.line_to(0, height-h_bar_h)
                else:
                    ctx.line_to(v_bar_w, end_y)
                    ctx.line_to(0, end_y)
    ...
    def _draw_secondary_level_bar(self, ctx, x, y, w, h, index, data):
        ...
        self._level_bar_drawer.update_display(data.get("raw_data"))
