...
class LevelBarDisplayer(DataDisplayer):
    ...
    def _animation_tick(self):
        ...
        # Animate the bar filling up/down
        ...
        # Always queue a draw to handle time-based effects like fading
    ...
    def on_draw(self, area, ctx, width, height):
        ...
        if label_orientation == "vertical":
            text_block_width = max(p_width, s_width)
            text_block_height = p_height + s_height
        else: # Horizontal
            text_block_width = p_width + s_width
            text_block_height = max(p_height, s_height)
        
        if show_primary and show_secondary:
            if label_orientation == "vertical":
                text_block_height += spacing
            else:
                text_block_width += spacing
        ...
        if text_pos != "superimposed":
            self._draw_external_text(ctx, width, height, text_pos, text_block_width, text_block_height, layout_p, layout_s)
        else:
            self._draw_superimposed_text(ctx, bar_x, bar_y, bar_width, bar_height, layout_p, layout_s)
    ...
    def _draw_external_text(self, ctx, width, height, position, text_block_width, text_block_height, layout_p, layout_s):
        ...
        if position == "top":
            ...
            text_area_y = height - text_block_height - padding
            text_area_x = 0
        elif position == "left":
            text_area_x = padding
            text_area_y = (height - text_block_height) / 2
        elif position == "right":
            text_area_x = width - text_block_width - padding
            text_area_y = (height - text_block_height) / 2
    ...
    def _draw_superimposed_text(self, ctx, bar_x, bar_y, bar_width, bar_height, layout_p, layout_s):
        ...
        show_primary = layout_p is not None
        show_secondary = layout_s is not None
    ...
    def _draw_label_set(self, ctx, area_x, area_y, area_width, area_height, layout_p, layout_s):
        ...
        if orientation == "vertical":
            ...
            if show_primary:
                ...
                layout_p.set_width(area_width * Pango.SCALE)
            ...
            if show_secondary:
                ...
                layout_s.set_width(area_width * Pango.SCALE)
    ...
    def draw_bar(self, ctx, bar_x, bar_y, bar_width, bar_height):
        ...
        # Draw the main background for the bar area
        bg_rgba = Gdk.RGBA()
        bg_rgba.parse(self.config.get("level_bar_background_color"))
        ctx.set_source_rgba(bg_rgba.red, bg_rgba.green, bg_rgba.blue, bg_rgba.alpha)
        ctx.rectangle(bar_x, bar_y, bar_width, bar_height)
        ctx.fill()
        
        # --- FIX: Handle zero spacing by drawing a single bar ---
        if spacing == 0:
            ...
            # Calculate the size of the "on" part of the bar
            on_ratio = self.current_on_level / num_segments if num_segments > 0 else 0
            
            if orientation == "vertical":
                on_height = bar_height * on_ratio
                off_height = bar_height - on_height
                ...
                ctx.rectangle(bar_x, bar_y, bar_width, off_height)
                ctx.fill()
                ...
                if on_height > 0:
                    if on_grad_enabled:
                        ...
                        c1 = Gdk.RGBA(); c1.parse(on_color1_str)
                        c2 = Gdk.RGBA(); c2.parse(on_color2_str)
                        pat.add_color_stop_rgba(0, c1.red, c1.green, c1.blue, c1.alpha)
                        pat.add_color_stop_rgba(1, c2.red, c2.green, c2.blue, c2.alpha)
                        ctx.set_source(pat)
                    else:
                        on_rgba = Gdk.RGBA(); on_rgba.parse(on_color1_str)
                        ctx.set_source_rgba(on_rgba.red, on_rgba.green, on_rgba.blue, on_rgba.alpha)
                    ctx.rectangle(bar_x, bar_y + off_height, bar_width, on_height)
                    ctx.fill()
            else: # Horizontal
                on_width = bar_width * on_ratio
                off_width = bar_width - on_width
                
                # Draw "off" part
                off_rgba = Gdk.RGBA(); off_rgba.parse(off_color_str)
                ctx.set_source_rgba(off_rgba.red, off_rgba.green, off_rgba.blue, off_rgba.alpha)
                ctx.rectangle(bar_x + on_width, bar_y, off_width, bar_height)
                ctx.fill()
                ...
                if on_width > 0:
                    if on_grad_enabled:
                        ...
                        on_rgba = Gdk.RGBA(); on_rgba.parse(on_color1_str)
                        ctx.set_source_rgba(on_rgba.red, on_rgba.green, on_rgba.blue, on_rgba.alpha)
                    ctx.rectangle(bar_x, bar_y, on_width, bar_height)
                    ctx.fill()
            return # We are done drawing for the zero-spacing case

        # --- Original logic for spaced segments ---
        ...
        for i in range(num_segments):
            if i < self.current_on_level: 
                ...
                if on_grad_enabled and self.current_on_level > 1:
                    gradient_factor = i / (self.current_on_level - 1)
                    base_color_str = self._interpolate_color(gradient_factor, on_color1_str, on_color2_str).to_string()
                ...
                if on_pulse_enabled:
                    ...
                    if pulse_duration_s > 0:
                        pulse_speed = (2 * math.pi) / pulse_duration_s
                        fade_factor = (math.sin(now * pulse_speed) + 1) / 2.0
                        
                        pulse_target_color_str = pulse_color1_str
                        if on_grad_enabled and self.current_on_level > 1:
                            gradient_factor = i / (self.current_on_level - 1)
                            pulse_target_color_str = self._interpolate_color(gradient_factor, pulse_color1_str, pulse_color2_str).to_string()
                ...
                if fade_enabled and not state['is_on'] and state['off_timestamp'] > 0:
                    ...
                    if elapsed < fade_duration_s: color_to_use = self._interpolate_color(elapsed/fade_duration_s, on_color1_str, off_color_str).to_string()
