...
class DashboardComboDisplayer(ComboBase):
    ...
    def __init__(self, panel_ref, config):
        # Pre-initialize drawer instances to be reused
        self._drawers = {
    ...
    def _get_full_config_model(cls):
        ...
        # Satellite display style models
        sat_display_opts = {
    ...
    def _calculate_center_positions(self, width, height):
        ...
        if count == 1:
            w_ratio = float(self.config.get("center_1_width_ratio", 1.0))
            item_w = min(width, height) * w_ratio
            positions["center_1"] = {'x': (width - item_w)/2, 'y': (height-item_w)/2, 'w': item_w, 'h': item_w}
        elif count == 2:
            w_ratio1 = float(self.config.get("center_1_width_ratio", 1.0))
            w_ratio2 = float(self.config.get("center_2_width_ratio", 1.0))
            item1_w = (width / 2) * w_ratio1
            item2_w = (width / 2) * w_ratio2
            positions["center_1"] = {'x': width/4 - item1_w/2, 'y': (height-item1_w)/2, 'w': item1_w, 'h': item1_w}
            positions["center_2"] = {'x': 3*width/4 - item2_w/2, 'y': (height-item2_w)/2, 'w': item2_w, 'h': item2_w}
        elif count == 3:
            w_ratio1 = float(self.config.get("center_1_width_ratio", 1.0))
            w_ratio2 = float(self.config.get("center_2_width_ratio", 1.0))
            w_ratio3 = float(self.config.get("center_3_width_ratio", 1.0))
            item1_w = (width / 3) * w_ratio1
            item2_w = (width / 3) * w_ratio2
            item3_w = (width / 3) * w_ratio3
            positions["center_1"] = {'x': width/6 - item1_w/2, 'y': (height-item1_w)/2, 'w': item1_w, 'h': item1_w}
            positions["center_2"] = {'x': width/2 - item2_w/2, 'y': (height-item2_w)/2, 'w': item2_w, 'h': item2_w}
            positions["center_3"] = {'x': 5*width/6 - item3_w/2, 'y': (height-item3_w)/2, 'w': item3_w, 'h': item3_w}
        elif count == 4:
            w_ratio1 = float(self.config.get("center_1_width_ratio", 1.0))
            w_ratio2 = float(self.config.get("center_2_width_ratio", 1.0))
            w_ratio3 = float(self.config.get("center_3_width_ratio", 1.0))
            w_ratio4 = float(self.config.get("center_4_width_ratio", 1.0))
            item1_w = (width/2) * w_ratio1; item1_h = (height/2) * w_ratio1
            item2_w = (width/2) * w_ratio2; item2_h = (height/2) * w_ratio2
            item3_w = (width/2) * w_ratio3; item3_h = (height/2) * w_ratio3
            item4_w = (width/2) * w_ratio4; item4_h = (height/2) * w_ratio4
            positions["center_1"] = {'x': width/4 - item1_w/2, 'y': height/4 - item1_h/2, 'w': item1_w, 'h': item1_h}
            positions["center_2"] = {'x': 3*width/4 - item2_w/2, 'y': height/4 - item2_h/2, 'w': item2_w, 'h': item2_h}
            positions["center_3"] = {'x': width/4 - item3_w/2, 'y': 3*height/4 - item3_h/2, 'w': item3_w, 'h': item3_h}
            positions["center_4"] = {'x': 3*width/4 - item4_w/2, 'y': 3*height/4 - item4_h/2, 'w': item4_w, 'h': item4_h}
        return positions
    ...
    def _calculate_satellite_positions(self, width, height, center_positions):
        ...
        for i in range(1, count + 1):
            ...
            size = float(self.config.get(f"{prefix}_size", 100))
            ...
            x = avg_center_x + math.cos(angle_rad) * distance_pixels - size/2
            y = avg_center_y + math.sin(angle_rad) * distance_pixels - size/2
            positions[prefix] = {'x': x, 'y': y, 'w': size, 'h': size}
        return positions
    ...
    def _draw_sub_display(self, ctx, x, y, w, h, prefix, data_bundle):
        ...
        # 1. Build the drawer's specific config from the main panel's config
        drawer_config = {}
        drawer_model = drawer.get_config_model()
        all_options = [opt for section in drawer_model.values() for opt in section]
        for option in all_options:
            prefixed_key = f"{prefix}_{option.key}"
            if prefixed_key in self.config:
                drawer_config[option.key] = self.config[prefixed_key]
            else:
                drawer_config[option.key] = option.default
        drawer.config = drawer_config

        # 2. Get the correct data packet and child data source instance
        source_key = f"{prefix}_source"
    ...
    def _animation_tick(self):
        ...
        # Redrawing is now implicitly handled by the child drawers' own animation ticks
        # and update_display calls. We just need to queue a draw on the main container.
