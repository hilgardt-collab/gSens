...
class GraphDisplayer(DataDisplayer):
    ...
    def get_config_model():
        ...
        model["Graph Visuals"] = [
            ...
            ConfigOption("graph_background_color", "color", "Background:", "rgba(34,34,34,1)"),
    ...
    def get_configure_callback(self):
        ...
        def setup_dynamic_options(dialog, content_box, widgets, available_sources, panel_config):
            build_ui_from_model(content_box, panel_config, self.get_config_model(), widgets)
    ...
    def on_draw_graph(self, area, ctx, width, height):
        ...
        bg_color_str = self.config.get(self.panel_ref.data_source.alarm_config_prefix + "alarm_color", "rgba(255,0,0,0.6)") if is_alarm else self.config.get("graph_background_color", "rgba(34,34,34,1)")
        bg_rgba = Gdk.RGBA(); bg_rgba.parse(bg_color_str); ctx.set_source_rgba(bg_rgba.red, bg_rgba.green, bg_rgba.blue, bg_rgba.alpha); ctx.paint()
        ...
        if len(self.history) >= 2:
            ...
            # --- FIX: Calculate X positions based on max_history_points for a scrolling effect ---
            max_hist = int(self.config.get("max_history_points", 100))
            num_points = len(values)
            
            # The width of a single data point's "slot"
            x_step = width / (max_hist - 1) if max_hist > 1 else width
            
            # The starting X coordinate on the canvas, creating the right-aligned effect
            ...
            if graph_type == "bar":
                # Bar width is now fixed based on the max history size
                bar_width = x_step
                ...
                for p in points:
                    bar_x = p[0] - (bar_width / 2) # Center the bar on the point
                    ...
                    if str(self.config.get("graph_fill_enabled", "False")).lower() == 'true':
                        ...
                        ctx.rectangle(bar_x, p[1], bar_width, height - p[1])
                        ctx.fill()
                ...
                line_rgba = Gdk.RGBA(); line_rgba.parse(self.config.get("graph_line_color"))
                ctx.set_source_rgba(line_rgba.red, line_rgba.green, line_rgba.blue, line_rgba.alpha)
                ctx.set_line_width(float(self.config.get("graph_line_width", 2.0)))
                ctx.stroke()

        # --- Draw Text Overlay ---
        if str(self.config.get("show_overlay", "True")).lower() == 'true':
            layout = PangoCairo.create_layout(ctx)
            font_desc = Pango.FontDescription.from_string(self.config.get("overlay_font"))
