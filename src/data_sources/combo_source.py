    ...
    def get_configure_callback(self):
        ...
        def _build_arc_config_ui(dialog, content_box, widgets, available_sources, panel_config, source_opts):
            ...
            for i in range(1, 17):
                ...
                page_num = arc_notebook.append_page(scroll, Gtk.Label(label=f"Arc {i}"))
                arc_tabs_content.append(arc_notebook.get_nth_page(page_num))
            ...
            def on_arc_count_changed(spinner):
                count = spinner.get_value_as_int() if spinner else int(panel_config.get("combo_arc_count", 5))
        ...
        def _build_level_bar_config_ui(dialog, content_box, widgets, available_sources, panel_config, source_opts):
            ...
            for i in range(1, 13):
                scroll = Gtk.ScrolledWindow(hscrollbar_policy=Gtk.PolicyType.NEVER, vexpand=True, min_content_height=300)
                tab_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6, margin_top=10, margin_bottom=10, margin_start=10, margin_end=10); scroll.set_child(tab_box)
                page_num = bar_notebook.append_page(scroll, Gtk.Label(label=f"Bar {i}"))
                bar_tabs_content.append(bar_notebook.get_nth_page(page_num))
            ...
            def on_bar_count_changed(spinner):
                count = spinner.get_value_as_int() if spinner else int(panel_config.get("number_of_bars", 3))
        ...
        def _build_dashboard_config_ui(dialog, content_box, widgets, available_sources, panel_config, source_opts):
            ...
            def on_center_count_changed(spinner):
                count = spinner.get_value_as_int() if spinner else int(panel_config.get("dashboard_center_count", 1))
                for i, tab in enumerate(center_tabs): tab.set_visible(i < count)
            
            center_count_spinner = widgets.get("dashboard_center_count")
            if center_count_spinner:
                center_count_spinner.connect("value-changed", on_center_count_changed)
                GLib.idle_add(on_center_count_changed, center_count_spinner)
            else:
                on_center_count_changed(None)
            ...
            def on_satellite_count_changed(spinner):
                count = spinner.get_value_as_int() if spinner else int(panel_config.get("dashboard_satellite_count", 4))
                for i, tab in enumerate(satellite_tabs): tab.set_visible(i < count)
            
            satellite_count_spinner = widgets.get("dashboard_satellite_count")
            if satellite_count_spinner:
                satellite_count_spinner.connect("value-changed", on_satellite_count_changed)
                GLib.idle_add(on_satellite_count_changed, satellite_count_spinner)
            else:
                on_satellite_count_changed(None)
