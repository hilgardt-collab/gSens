...
class DataSource(ABC):
    ...
    def get_configure_callback(self):
        ...
        def setup_dynamic_alarm_options(dialog, content_box, widgets, available_sources, panel_config):
            prefix = self.alarm_config_prefix
            enable_widget = widgets.get(f"{prefix}enable_alarm")
            
            # Find all other alarm widgets to hide/show them
            alarm_widgets_to_toggle = []
            for key in widgets:
                if key.startswith(prefix) and key != f"{prefix}enable_alarm":
