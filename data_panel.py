...
class DataPanel(BasePanel):
    ...
    """
    def __init__(self, config, data_source, data_displayer):
    ...
    def configure(self, *args):
        ...
        AVAILABLE_DATA_SOURCES = getattr(main_window, 'AVAILABLE_DATA_SOURCES', {})
        ...
        def build_new_display_tab():
            ...
            current_display_model = self.data_displayer.get_config_model()
            
            custom_builder_callback = getattr(self.data_displayer, 'get_configure_callback', lambda: None)()
            
            # --- REFACTOR: Let custom callbacks handle UI creation entirely if they exist. ---
            if custom_builder_callback:
                custom_builder_callback(dialog, tab_box, all_widgets, AVAILABLE_DATA_SOURCES, self.config)
            else:
                # Otherwise, build the UI from the standard model.
                build_ui_from_model(tab_box, self.config, current_display_model, all_widgets)
        ...
        build_background_config_ui(panel_tab_box, self.config, all_widgets, dialog)
        ...
        def apply_changes(widget=None):
            ...
            nonlocal current_display_model
            current_display_model = self.data_displayer.get_config_model()
            background_model = dialog.ui_models.get('background', {})
            ...
            if hasattr(self.data_source, 'setup_child_sources'):
                self.data_source.setup_child_sources(AVAILABLE_DATA_SOURCES)
