...
def build_background_config_ui(parent_box, config, widgets, dialog_parent):
    """
    Builds and adds UI controls for selecting a background (solid, gradient, image)
    to a given parent container.
    """
    ...
    header_label.set_markup(f"<b>Background Settings</b>")
    parent_box.append(header_label)

    bg_model = {
        "bg_type": ConfigOption("bg_type", "dropdown", "Type:", "solid", options_dict={"Solid Color": "solid", "Linear Gradient": "gradient", "Image": "image"}),
        "bg_color": ConfigOption("bg_color", "color", "Color:", "#222222"),
        "gradient_color1": ConfigOption("gradient_color1", "color", "Start Color:", "#444444"),
        "gradient_color2": ConfigOption("gradient_color2", "color", "End Color:", "#222222"),
        "gradient_direction": ConfigOption("gradient_direction", "dropdown", "Direction:", "horizontal", options_dict={"Horizontal": "horizontal", "Vertical": "vertical"}),
        "background_image_path": ConfigOption("background_image_path", "string", "Image File:", ""),
        "background_image_style": ConfigOption("background_image_style", "dropdown", "Style:", "zoom", options_dict={"Zoom (Cover)": "zoom", "Tile": "tile", "Stretch": "stretch"}),
        "background_image_alpha": ConfigOption("background_image_alpha", "scale", "Image Opacity:", "1.0", min_val=0.0, max_val=1.0, step=0.05, digits=2),
    }
    
    if not hasattr(dialog_parent, 'ui_models'):
        dialog_parent.ui_models = {}
    dialog_parent.ui_models['background'] = {"Background Settings": list(bg_model.values())}

    build_ui_from_model(parent_box, config, {"": [bg_model["bg_type"]]}, widgets)
    
    bg_type_combo = widgets["bg_type"]

    solid_color_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6, margin_top=5)
    parent_box.append(solid_color_box)
    build_ui_from_model(solid_color_box, config, {"": [bg_model["bg_color"]]}, widgets)

    gradient_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6, margin_top=5)
    parent_box.append(gradient_box)
    build_ui_from_model(gradient_box, config, {"": [bg_model["gradient_color1"], bg_model["gradient_color2"], bg_model["gradient_direction"]]}, widgets)

    image_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6, margin_top=5)
    parent_box.append(image_box)
    
    image_path_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
    image_box.append(image_path_row)
    image_path_row.append(Gtk.Label(label="Image File:", xalign=0))
    
    image_path_entry = Gtk.Entry(text=config.get("background_image_path", ""), hexpand=True, editable=False)
    widgets["background_image_path"] = image_path_entry
    image_path_row.append(image_path_entry)

    def on_choose_image_clicked(_btn):
        file_chooser = Gtk.FileChooserNative.new("Select Background Image", dialog_parent, Gtk.FileChooserAction.OPEN, "_Open", "_Cancel")
        filter_img = Gtk.FileFilter.new()
        filter_img.set_name("Images")
        filter_img.add_mime_type("image/*")
        file_chooser.add_filter(filter_img)
        def on_file_chooser_response(native_dialog, response):
            if response == Gtk.ResponseType.ACCEPT:
                image_path_entry.set_text(native_dialog.get_file().get_path() or "")
            native_dialog.destroy()
        file_chooser.connect("response", on_file_chooser_response)
        file_chooser.show()

    choose_image_button = Gtk.Button(label="Choose…")
    choose_image_button.connect("clicked", on_choose_image_clicked)
    image_path_row.append(choose_image_button)

    build_ui_from_model(image_box, config, {"": [bg_model["background_image_style"], bg_model["background_image_alpha"]]}, widgets)

    def on_bg_type_changed(combo):
        active_id = combo.get_active_id()
        solid_color_box.set_visible(active_id == "solid")
        gradient_box.set_visible(active_id == "gradient")
