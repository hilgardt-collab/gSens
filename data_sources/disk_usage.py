...
class DiskUsageDataSource(DataSource):
    ...
    def __init__(self, config):
        ...
        self._temp_selected_mount_path = self.config.get("mount_path", "/")
    ...
    def get_configure_callback(self):
        ...
        def add_disk_chooser(dialog, content_box, widgets, available_sources, panel_config):
            ...
            self._temp_selected_mount_path = panel_config.get("mount_path", "/")
            ...
            widgets["current_mount_path_label"] = Gtk.Label(label=self._temp_selected_mount_path, xalign=0, hexpand=True, ellipsize=Pango.EllipsizeMode.MIDDLE)
            row.append(widgets["current_mount_path_label"])
            
            widgets["choose_mount_button"] = Gtk.Button(label="Choose…")
            widgets["choose_mount_button"].connect("clicked", self._show_mount_chooser, dialog, widgets)
            row.append(widgets["choose_mount_button"])
            
            dialog.custom_value_getter = lambda: {"mount_path": self._temp_selected_mount_path}

            if "disk_title_format" in widgets:
                widgets["disk_title_format"].connect("changed", lambda w: self._update_panel_title(widgets))
            
            GLib.idle_add(self._update_panel_title, widgets)

        return add_disk_chooser

    def _update_panel_title(self, widgets):
        """Updates the main panel title entry based on the format and selected path."""
        try:
            title_format_entry = widgets["disk_title_format"]
            title_entry = widgets["title_text"]
            
            format_str = title_format_entry.get_text()
            new_title = format_str.replace("{mount_point}", self._temp_selected_mount_path)
            title_entry.set_text(new_title)
        except KeyError as e:
            print(f"Warning: Could not find widget for auto-title: {e}")

    def _show_mount_chooser(self, btn, parent_dlg, widgets):
        path_label = widgets["current_mount_path_label"]
        ...
        for cat_key, cat_name in [("internal","Internal"),("external","External"),("other","Other")]:
            ...
            for item in sorted(partitions[cat_key], key=lambda x:x["mountpoint"]):
                ...
                if item["mountpoint"] == self._temp_selected_mount_path: list_box.select_row(row)
        ...
        if dlg.run() == Gtk.ResponseType.OK:
            for lb in list_boxes:
                if lb.get_selected_row():
                    self._temp_selected_mount_path = lb.get_selected_row().mountpoint_data
                    path_label.set_text(self._temp_selected_mount_path)
                    self._update_panel_title(widgets)
                    break
        dlg.destroy()
