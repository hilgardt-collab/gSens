...
class BasePanel(Gtk.Frame, BasePanelABC, metaclass=BasePanelMeta):
    ...
    def __init__(self, title="", config=None):
        ...
        self.config.setdefault("bg_type", "solid")
        self.config.setdefault("bg_color", "#222222")
        self.config.setdefault("gradient_color1", "#444444")
        self.config.setdefault("gradient_color2", "#222222")
        self.config.setdefault("gradient_direction", "horizontal")
        self.config.setdefault("background_image_path", "")
        self.config.setdefault("background_image_style", "zoom")
        self.config.setdefault("background_image_alpha", "1.0")
    ...
    def apply_panel_frame_style(self):
        ...
        if is_in_alarm:
            ...
            bg_type = self.config.get("bg_type", "solid")
            if bg_type == "image":
                image_path = self.config.get("background_image_path")
                if image_path and os.path.exists(image_path):
                    image_uri = GLib.filename_to_uri(image_path, None)
                    image_alpha = float(self.config.get("background_image_alpha", 1.0))
                    ...
                    css_parts.append(f"background-color: {self.config.get('bg_color', '#222222')};")
                    image_style = self.config.get("background_image_style", "zoom")
                    if image_style == "tile":
                ...
                grad_dir = "to bottom" if self.config.get("gradient_direction", "horizontal") == "vertical" else "to right"
                color1 = self.config.get('gradient_color1', '#444444')
                color2 = self.config.get('gradient_color2', '#222222')
                css_parts.append(f"background-image: linear-gradient({grad_dir}, {color1}, {color2});")
            else: # solid
                css_parts.append(f"background-color: {self.config.get('bg_color', '#222222')};")
