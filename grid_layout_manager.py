...
class GridLayoutManager(Gtk.Fixed):
    ...
    def __init__(self, available_sources, available_displayers, all_source_classes):
        ...
        grid_layout_config.setdefault("bg_type", "solid")
        grid_layout_config.setdefault("bg_color", "#333333")
        grid_layout_config.setdefault("gradient_color1", "#444444")
        grid_layout_config.setdefault("gradient_color2", "#222222")
        grid_layout_config.setdefault("gradient_direction", "horizontal")
        grid_layout_config.setdefault("background_image_path", "")
        grid_layout_config.setdefault("background_image_style", "zoom")
    ...
    def create_panel_widget(self, config_dict):
        ...
        # The backward-compatibility code for old data source types has been removed.
        ...
        if type_id in self.AVAILABLE_DATA_SOURCES:
            ...
            if disp_key and disp_key in self.AVAILABLE_DISPLAYERS:
                ...
                if hasattr(source, 'setup_child_sources'):
                    source.setup_child_sources(self.AVAILABLE_DATA_SOURCES)

                displayer = DisplayerClass(panel_ref=None, config=config_dict)
                return DataPanel(config=config_dict, data_source=source, data_displayer=displayer)
    ...
    def handle_copy_config_request(self, destination_id):
        ...
        keys_to_copy = [
            'width', 'height', 'bg_type', 'bg_color', 'gradient_color1', 'gradient_color2',
            'gradient_direction', 'background_image_path', 'background_image_style', 'background_image_alpha'
        ]
    ...
    def _load_and_apply_grid_config(self):
        ...
        bg_type = grid_config.get("bg_type", "solid")
        
        if bg_type == "image":
            image_path = grid_config.get("background_image_path")
            if image_path and os.path.exists(image_path):
                image_uri = GLib.filename_to_uri(image_path, None)
                css_rules.append(f"background-image: url('{image_uri}');")
                style = grid_config.get("background_image_style", "zoom")
            ...
            else: css_rules.append(f"background-color: {grid_config.get('bg_color', '#333333')};")
        elif bg_type == "gradient":
            direction = "to bottom" if grid_config.get("gradient_direction", "horizontal") == "vertical" else "to right"
            color1, color2 = grid_config.get("gradient_color1"), grid_config.get("gradient_color2")
            css_rules.append(f"background-image: linear-gradient({direction}, {color1}, {color2});")
        else: # solid
            css_rules.append(f"background-color: {grid_config.get('bg_color', '#333333')};")
    ...
    def _on_configure_layout_activate(self, action, param):
        ...
        build_background_config_ui(main_box, dict(config_manager.config["GridLayout"]), all_widgets, dialog)
        dialog.ui_models['layout'] = layout_config_model

        def apply_changes(widget=None):
            new_background_conf = get_config_from_widgets(all_widgets, [dialog.ui_models.get('background', {})])
            ...
            for key, value in {**new_background_conf, **new_layout_conf}.items():
